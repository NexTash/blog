  ...
  <main
    ...
    <div class="mx-auto max-w-6xl">
      ...
      <div v-if="isSubmitted" class="fade-in flex flex-col items-center justify-center min-h-[70vh]">
        <div class="max-w-xl w-full bg-white rounded-3xl p-12 border-2 border-gray-300 shadow-2xl text-center">
          <div
          ...
          <h2 class="text-4xl font-serif font-bold text-black mb-6">Submission Received!</h2>
          <p class="text-lg text-gray-600 mb-10 leading-relaxed">
            Your blog is successfully submitted and <span
              class="font-bold text-black underline decoration-red-500 underline-offset-4">waiting for admin
              approval</span>.
            Once the team reviews and publishes your story, it will appear on the public feed.
          </p>
      ...
      <div v-else>
        ...
        <header
          ...
          <div>
            <nav class="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500 mb-2">
              <span>Blogger</span>
              <span>/</span>
              <span class="text-black">New Story</span>
            </nav>
            <h1 class="text-3xl md:text-4xl font-serif font-bold tracking-tight text-black">Create New Story</h1>
          </div>

          <!-- Tab Bar -->
          <div class="inline-flex rounded-full bg-gray-200 p-1 shadow-inner">
            <button type="button" @click="activeTab = 'write'"
              :class="activeTab === 'write' ? 'bg-black text-white shadow-md' : 'text-gray-600 hover:text-black'"
            ...
            <button type="button" @click="activeTab = 'preview'"
              :class="activeTab === 'preview' ? 'bg-black text-white shadow-md' : 'text-gray-600 hover:text-black'"
              class="flex items-center gap-2 rounded-full px-6 py-2 text-xs font-bold uppercase tracking-wider transition-all">
              👁️ Preview
            </button>
          </div>
        ...
        <div v-show="activeTab === 'write'" class="fade-in">
          <form @submit.prevent="handleSubmit" class="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_340px]">
            ...
            <div class="space-y-8">
              ...
              <div
                ...
                <button v-else @click="$refs.imageInput.click()" type="button"
                  class="flex h-full w-full flex-col items-center justify-center gap-4 transition-colors hover:bg-gray-50">
                  <div class="rounded-full bg-gray-100 p-4 text-black border border-gray-300">
                  ...
                  <div class="text-center">
                    <p class="text-sm font-bold text-black uppercase tracking-widest">Add Cover Image</p>
                    <p class="text-[11px] text-gray-500 mt-1 font-medium">PNG, JPG, WebP (Max 5MB)</p>
                  </div>
                </button>
              ...
              <div
                ...
                <label class="mb-4 block text-[11px] font-black uppercase tracking-[0.2em] text-black">Story
                  Title</label>
                <input v-model="form.title" type="text" placeholder="Enter a compelling title..."
              ...
              <div
                ...
                <div
                  ...
                  <span class="bg-black text-white px-2 py-0.5 rounded">{{ wordCount }} Words</span>
                </div>
                <textarea v-model="form.content" rows="18" placeholder="Write your story here..."
                  class="w-full border-none bg-transparent p-8 font-mono text-[15px] leading-relaxed placeholder:text-gray-300 focus:ring-0 focus:outline-none text-black"></textarea>
              </div>

              <!-- Excerpt -->
              <div
                class="bg-white rounded-2xl p-8 border-2 border-gray-300 shadow-md transition-shadow hover:shadow-lg focus-within:border-black">
                <label class="mb-4 block text-[11px] font-black uppercase tracking-[0.2em] text-black">Short Summary /
                  Excerpt</label>
                <textarea v-model="form.blog_intro" rows="3"
                  placeholder="What is this story about? (Summarize in 2-3 sentences)"
                  class="w-full border-none p-0 text-lg leading-relaxed text-gray-800 placeholder:text-gray-300 focus:ring-0 focus:outline-none"></textarea>
              </div>
            </div>
            ...
            <aside class="space-y-6">
              <div class="sticky top-8 space-y-6">
                ...
                <div class="rounded-2xl p-6 bg-black text-white shadow-xl">
                  ...
                  <p v-if="!isFormValid" class="text-[10px] mt-4 text-center text-gray-500 italic">Please fill title and
                    content to enable publishing.</p>
                </div>

                <!-- Category Card -->
                <div class="rounded-2xl bg-white p-6 border-2 border-gray-300 shadow-md">
                  <label
                    class="mb-4 block text-[11px] font-black uppercase tracking-[0.2em] text-black">Category</label>
                  <div class="relative">
                    <select v-model="form.category"
                      ...
                      <option v-for="cat in categories" :key="cat.name" :value="cat.name">{{ cat.title || cat.name }}
                      </option>
                    </select>
                  </div>
                </div>
                ...
                <!-- NEW: Backlinks Card -->
                <div class="rounded-2xl bg-white p-6 border-2 border-gray-300 shadow-md">
                  <label class="mb-4 block text-[11px] font-black uppercase tracking-[0.2em] text-black">Backlinks /
                    Resources</label>
                  <div class="space-y-2 mb-4">
                    <input v-model="backlinkInput.label" placeholder="Link Label (e.g. Source)"
                    ...
                    <input v-model="backlinkInput.url" placeholder="https://example.com"
                      class="w-full rounded-lg border-2 border-gray-100 bg-gray-50 px-3 py-2 text-[11px] font-bold text-black focus:border-black focus:outline-none" />
                    <button @click.prevent="addBacklink" type="button"
                  ...
                  <div class="space-y-2">
                    <div v-for="(link, i) in form.backlinks" :key="i"
                      class="group flex items-center justify-between bg-gray-50 border border-gray-200 p-2 rounded-lg">
                      <div class="overflow-hidden">
                        <p class="text-[10px] font-bold text-black truncate">{{ link.label }}</p>
                        <p class="text-[9px] text-gray-400 truncate">{{ link.url }}</p>
                      </div>
                      <button @click="removeBacklink(i)" type="button"
                        class="text-gray-300 hover:text-red-500 px-2 transition-colors">×</button>
                    </div>
                  </div>
                </div>
                ...
                <!-- Tags Card -->
                <div class="rounded-2xl bg-white p-6 border-2 border-gray-300 shadow-md">
                  <label class="mb-4 block text-[11px] font-black uppercase tracking-[0.2em] text-black">Tags</label>
                  <input v-model="tagInput" @keydown.enter.prevent="addTag" placeholder="Type and press Enter"
                  ...
                  <div class="flex flex-wrap gap-2">
                    <span v-for="(tag, i) in form.tags" :key="tag"
                      ...
                      <button @click="removeTag(i)" type="button"
                        class="text-gray-400 hover:text-red-500 transition-colors">×</button>
                    </span>
                  </div>
                </div>
        ...
        <div v-show="activeTab === 'preview'" class="mx-auto max-w-3xl fade-in">
          <div class="bg-white min-h-[80vh] rounded-3xl shadow-2xl overflow-hidden mb-20 border-2 border-gray-200">
            <div v-if="imagePreview" class="h-80 w-full overflow-hidden bg-gray-900">
              <img :src="imagePreview" alt="Cover" class="h-full w-full object-cover" />
            </div>
            <div class="px-8 md:px-16 py-12">
              <div class="mb-8 flex items-center gap-4">
                ...
                <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ today }}</span>
              </div>
              <h1 class="font-serif text-4xl md:text-6xl text-black font-bold mb-8 leading-tight">{{ form.title ||
                "Untitled Story" }}</h1>
              ...
              <div class="preview-content-area mb-12"
                v-html="form.content || '<p class=\'text-gray-300 italic\'>Write some content to see it here...</p>'">
              </div>

              <!-- Preview Backlinks -->
              <div v-if="form.backlinks.length > 0" class="border-t-2 border-gray-100 pt-8">
                <h4 class="text-[11px] font-black uppercase tracking-[0.2em] mb-4">Resources & Links</h4>
                <ul class="space-y-2">
...
const getServerMessage = (data) => {
  if (data._server_messages) {
    ...
    } catch (e) { return "Server Error"; }
  }
  return data.exception || data.message || "An unexpected error occurred.";
};
...
const today = computed(() => new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }));
...
const fetchCategories = async () => {
  try {
    const res = await fetch("/api/method/blog.api.blog_categories");
...
const handleSubmit = async () => {
  ...
  try {
    ...
    formData.append("blog_intro", form.blog_intro);
    formData.append("content", form.content);
    formData.append("category", form.category);
    formData.append("tags", JSON.stringify(form.tags));

    // Send the array of objects to the backend
    formData.append("backlinks", JSON.stringify(form.backlinks));
    ...
    if (!response.ok) {
      throw new Error(getServerMessage(data));
    }
