	...
	>
		<div class="p-6 md:p-8">
			...
			<div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
				<div>
					<span
						class="text-blue-600 font-bold text-xs uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-full"
						>Partners</span
					>
					<h2
						class="text-3xl font-extrabold mt-2 text-gray-900 tracking-tight flex items-center gap-3"
					>
						Sponsored Links
			...
			<!-- Ads Grid -->
			<div
				v-else-if="adsResource.data && adsResource.data.length > 0"
				class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8"
			>
				<a
					v-for="ad in adsResource.data"
					:key="ad.name"
					:href="getSafeUrl(ad.link)"
					target="_blank"
				...
				>
					...
					<div
						class="relative aspect-video overflow-hidden rounded-2xl z-0 shadow-sm border border-gray-100"
					>
						<img
							v-if="ad.image"
			...
			<div
				v-else
				class="text-center py-16 border-2 border-dashed border-gray-200 rounded-3xl"
			>
