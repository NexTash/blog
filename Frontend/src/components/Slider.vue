<template>
	<div class="border-b border-gray-200 bg-[#f6f3ee] px-4 py-3">
		...
		>
			...
			<div
				class="flex h-[50px] flex-1 items-center overflow-hidden bg-white min-[575px]:h-full"
			>
				<div
					v-if="blogs.data && blogs.data.length > 0"
				...
				>
					<ul class="m-0 flex list-none items-center p-0">
						<li
							...
							:key="'a-' + item.name"
							class="px-6 min-[575px]:px-8"
						>
							<router-link
								:to="{ name: 'PostDetail', params: { name: item.name } }"
